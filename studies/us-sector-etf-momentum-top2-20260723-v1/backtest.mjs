...
const SECTOR_ETFS = ["XLK", "XLF", "XLE", "XLV", "XLI", "XLY", "XLP", "XLU", "XLC", "XLRE", "XLB"];
const ALL_SYMBOLS = ["QQQ", ...SECTOR_ETFS];
...
function buildCohorts(priceMap, asOf) {
  const qqqRows = priceMap.get("QQQ") ?? [];
  const firstCommonDate = SECTOR_ETFS
  ...
  for (let year = first.getUTCFullYear(); year <= last.getUTCFullYear(); year += 1) {
    ...
    for (let month = startMonth; month <= endMonth; month += 1) {
      ...
      const ranked = SECTOR_ETFS.map((symbol) => {
      ...
      if (ranked.length !== SECTOR_ETFS.length) continue;
...
function buildTradePlans(priceMap, cohorts, asOf) {
  const weeklyMap = new Map(SECTOR_ETFS.map((symbol) => [symbol, weeklyRows(priceMap.get(symbol) ?? [])]));
...
function simulate(priceMap, cohorts, tradePlans, costRate) {
  const account = {
  ...
  const selectionCounts = Object.fromEntries(SECTOR_ETFS.map((symbol) => [
    symbol,
...
function report(result) {
  ...
  const difference = strategy.totalReturn - benchmark.totalReturn;
  const winner = difference > 0 ? "섹터 ETF 전략" : "QQQ 계속 보유";
  const lines = [
    "# 미국 섹터 ETF 모멘텀 상위 2종 전략 백테스트",
    "",
    `결론부터 말하면, 같은 기간에는 **${winner}**가 앞섰습니다. 섹터 ETF 전략은 ${pct(strategy.totalReturn)}, QQQ 계속 보유는 ${pct(benchmark.totalReturn)}로 차이는 ${pct(difference)}p입니다.`,
    "",
    "## 비교 결과",
    "",
    "| 항목 | 섹터 ETF 상위 2종 | QQQ 계속 보유 |",
    "|---|---:|---:|",
    `| 시작 자금 | ${money(strategy.initialCapital)} | ${money(benchmark.initialCapital)} |`,
    `| 마지막 자산 | ${money(strategy.finalCapital)} | ${money(benchmark.finalCapital)} |`,
    `| 누적 수익률 | ${pct(strategy.totalReturn)} | ${pct(benchmark.totalReturn)} |`,
    `| 연평균 복리수익률 | ${pct(strategy.cagr)} | ${pct(benchmark.cagr)} |`,
    `| 최대 낙폭 | ${pct(strategy.maxDrawdown.value)} | ${pct(benchmark.maxDrawdown.value)} |`,
    "",
    `최종 자산 차이는 ${money(benchmark.finalCapital - strategy.finalCapital)}로 QQQ가 더 많았습니다. 섹터 전략은 최대 낙폭을 ${pct(Math.abs(benchmark.maxDrawdown.value) - Math.abs(strategy.maxDrawdown.value))}p 줄였지만, 수익 차이를 보상할 정도는 아니었습니다.`,
    "",
    `기간은 ${strategy.firstDate}부터 ${strategy.lastDate}까지이며, 11개 ETF 모두가 126거래일 모멘텀을 계산할 수 있는 첫 신호부터 시작했습니다.`,
    "",
    "## 무엇을 사고팔았나",
    "",
  ...
  for (const row of result.main.annualComparison) {
    lines.push(`| ${row.year} | ${pct(row.strategy)} | ${pct(row.qqq)} | ${pct(row.strategy - row.qqq)}p |`);
  }
  lines.push(
    "",
  ...
  for (const row of result.main.startDateSensitivity) {
    lines.push(`| ${row.startYear} | ${pct(row.strategyReturn)} | ${pct(row.qqqReturn)} | ${pct(row.excessReturn)}p |`);
  }
  lines.push(
    "",
  ...
  for (const row of result.main.feeSensitivity) {
    lines.push(`| ${(row.costEachSide * 100).toFixed(2)}% | ${pct(row.strategyReturn)} | ${pct(row.qqqReturn)} | ${pct(row.excessReturn)}p |`);
  }
  lines.push(
    "",
  ...
  lines.push(
    "",
    `XLK가 가장 많이 뽑혔지만 ${strategy.selectionCounts.XLK}회, 전체 lot의 ${pct(strategy.selectionCounts.XLK / strategy.attemptedBuys)}에 그쳤습니다. 이 기간 QQQ를 크게 끌어올린 대형 기술주 상승을 섹터 순환 전략이 일부만 담았다는 뜻입니다.`,
    ""
  );
  lines.push(
    "## 최근 12개월 선정",
...
async function main() {
  ...
  const cohorts = buildCohorts(priceMap, asOf);
  if (cohorts.length < 12) throw new Error(`Too few valid cohorts: ${cohorts.length}`);
  const trades = buildTradePlans(priceMap, cohorts, asOf);

  const strategy = simulate(priceMap, cohorts, trades, MAIN_COST_RATE);
  ...
  const feeSensitivity = [0.001, 0.0025, 0.005].map((costRate) => {
    const scenario = simulate(priceMap, cohorts, trades, costRate);
  ...
  const startDateSensitivity = [2019, 2020, 2021, 2022, 2023, 2024].map((year) => {
    ...
    const subsetTrades = buildTradePlans(priceMap, subset, asOf);
    const scenario = simulate(priceMap, subset, subsetTrades, MAIN_COST_RATE);
  ...
  const result = {
    ...
    studyId: "us-sector-etf-momentum-top2-20260723-v1",
    ...
    audit: {
      ...
      allRankingsHaveElevenEtfs: cohorts.every((cohort) => cohort.ranking.length === 11),

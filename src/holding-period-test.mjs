...
const outputJsonPath = path.join("data", "holding-period-test.json");
const outputMdPath = "holding_period_test.md";
const sample = process.argv.includes("--sample");
const costBps = Number(valueAfter("--cost-bps") ?? 10);
const holdingMonths = [1, 2, 3, 6];
...
function benchmarkReturn(priceMap, symbol, startDate, endDate) {
  return periodReturn(priceMap, symbol, startDate, endDate);
}
...
function simulateHolding(priceMap, strategy, holdMonths) {
  let equity = 1;
  let spyEquity = 1;
  let qqqEquity = 1;
  const curve = [];
  ...
  const maxIndex = allPeriods.length - 1;
  ...
  for (let index = 0; index < maxIndex; index += 1) {
    ...
    for (let offset = 0; offset < holdMonths; offset += 1) {
      ...
      const cohort = allPeriods[cohortIndex];
      if (!cohort.symbols?.length) continue;
    ...
    const spyReturn = benchmarkReturn(priceMap, "SPY", current.entryDate, next.entryDate);
    const qqqReturn = benchmarkReturn(priceMap, "QQQ", current.entryDate, next.entryDate);
...
function table(lines, rows) {
  lines.push("| Strategy | Hold | Months | Total | CAGR | QQQ Total | Excess QQQ | MDD | Avg Month | Positive | Beat QQQ | Vol | Cost |");
  lines.push("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|");
  for (const row of rows) {
    lines.push(`| ${row.strategyLabel} | ${row.holdingMonths}M | ${row.months} | ${formatPct(row.totalReturn)} | ${formatPct(row.cagr)} | ${formatPct(row.qqqTotalReturn)} | ${formatPct(row.excessQqqTotal)} | ${formatPct(row.maxDrawdown)} | ${formatPct(row.averageMonthlyReturn)} | ${formatPct(row.positiveMonthRate)} | ${formatPct(row.beatQqqMonthRate)} | ${formatPct(row.annualizedVolatility)} | ${formatPct(row.totalTransactionCost)} |`);
  }
}

function markdown(result) {
  const lines = [];
  lines.push("# Holding Period Test");
  lines.push("");
  ...
  lines.push("");
  lines.push("## Best Holding Period By Strategy");
  lines.push("");
  table(lines, result.bestByStrategy);
  lines.push("");
  lines.push("## Main Strategy Holding Comparison");
  lines.push("");
  ...
  for (const row of result.results
    ...
    .sort((a, b) => a.holdingMonths - b.holdingMonths)) {
    lines.push(`### Hold ${row.holdingMonths}M`);
    lines.push("");
    lines.push("| As Of | Net | QQQ | Cohorts | Equity | Newest Groups |");
    lines.push("|---|---:|---:|---:|---:|---|");
    for (const period of row.curve.slice(-6)) {
      lines.push(`| ${period.asOf} | ${formatPct(period.netReturn)} | ${formatPct(period.qqqReturn)} | ${period.activeCohorts} | ${formatNumber(period.equity)} | ${period.newestGroups.join(", ")} |`);
    }
    lines.push("");
  }
  lines.push("## Notes");
  lines.push("");
  lines.push("- Holding periods above 1M are simulated as overlapping monthly cohorts.");
  lines.push("- Each active cohort has equal capital weight. A 3M hold means roughly three monthly baskets are active together.");
...
async function main() {
  ...
  console.log(sample ? "Running holding-period test with sample data." : "Running holding-period test with live data.");
  ...
  const results = strategies.flatMap((strategy) => holdingMonths.map((months) => simulateHolding(priceMap, strategy, months)));
  const result = {
